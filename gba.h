...
#define IRQ_TIMER(x) (1 << ((x & 3) + 3)) // Enables timer x interrupts, requires bit 6 in REG_TMxCNT
#define IRQ_DMA(x)   (1 << ((x & 3) + 8)) // Enables DMA x interrupts, requires that DMA channel's register
#define IRQ_BUTTON   (1 << 12) // Enables button interrupts, requires bit E (and button specifications) in REG_KEYCNT

// Checks if an interrupt has been triggered
#define INTERRUPT_FLAGGED(flag) /* Add from Lab10 */
